...
// Load and deduplicate city names once
...
// Debounced autocomplete input handler
cityInput.addEventListener("input", () => {
  clearTimeout(debounceTimeout);
  debounceTimeout = setTimeout(handleAutocomplete, 300); // 300ms debounce
});
...
async function handleAutocomplete() {
  ...
  // Limit candidates to 20 to reduce API calls
  candidates = candidates.slice(0, 20);

  // Validate only top 10 cities with backend to minimize lag
  const validCities = [];
